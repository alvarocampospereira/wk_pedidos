...
begin
  qryBase.Close;
  qryBase.SQL.Clear;
  qryBase.SQL.Add('SELECT ID_PEDIDO, ID_CLIENTE, DATAEMISSAO, VALORTOTAL FROM PEDIDO WHERE ID_PEDIDO = ' + IntToStr(objPedido.id_Pedido));
  qryBase.Open;
  Result := objPedido;
...
begin
  try
    ...
    qryBase.Close;
    qryBase.SQL.Clear;
    qryBase.SQL.Add('INSERT PEDIDO (ID_PEDIDO, ID_CLIENTE, DATAEMISSAO, VALORTOTAL) VALUES (:ID_PEDIDO, :ID_CLIENTE, :DATAEMISSAO, :VALORTOTAL)');
    qryBase.SQL.Add('ON DUPLICATE KEY UPDATE ID_CLIENTE = :ID_CLIENTE, DATAEMISSAO = :DATAEMISSAO, VALORTOTAL = :VALORTOTAL');
...
begin
  try
    qryBase.Close;
    qryBase.SQL.Clear;
    qryBase.SQL.Add('DELETE FROM PEDIDO WHERE ID_PEDIDO = ' + IntToStr(id));
    qryBase.ExecSQL;
    Result := True;
  except on E: Exception do
  begin

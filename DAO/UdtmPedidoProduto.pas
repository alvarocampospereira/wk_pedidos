...
function TdtmPedidoProduto.ExcluirTodosProdutosPedido(id: Integer): Boolean;
begin
  Result := False;
  try
    qryBase.Close;
    qryBase.SQL.Clear;
    qryBase.SQL.Add('DELETE FROM PEDIDOPRODUTO WHERE ID_PEDIDO = ' + IntToStr(id));
    qryBase.ExecSQL;
    Result := True;
  except on E: Exception do
  begin
    raise Exception.Create('Erro ao excluir todos pedido produto. Mensagem:' + e.Message);
  end;
  end;
end;
